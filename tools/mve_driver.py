"""Run a registered PolicyOfficer/PolicyCaseEnv on official AgentSociety + Ray."""
import argparse
import asyncio
...
async def run(args, report):
    ...
    try:
        ...
        actor = remote_class.remote(str(args.run_dir), args.policy, args.seed, args.horizon, args.mode, client)
        env = EnvRouterProxy(actor, run_dir=args.run_dir, env_module_types=["PolicyCaseEnv"])
        proxy.env = env
        if args.resume:
            ...
            society = AuditedSociety(agent_specs=specs, agent_class_name="PolicyOfficer", env_router=env,
                        start_t=datetime(2026, 1, 1, 9), run_dir=args.run_dir, service_proxy=proxy,
                        batch_size=1, enable_replay=True, env_module_types=["PolicyCaseEnv"],
                        env_kwargs={"PolicyCaseEnv": {"policy": args.policy, "seed": args.seed, "horizon": args.horizon}})
            await society.init()
...
def main():
    ...
    parser.add_argument("--policy", default="111")
    ...
    if args.steps < 0 or (not args.resume and args.steps == 0) or args.steps > args.horizon:
        raise SystemExit("Steps must fit the horizon; zero is only for a restore check")
    os.chdir(ROOT)
    args.run_dir = args.run_dir.resolve()
    if not args.run_dir.is_relative_to(ROOT / "runs"):
        raise SystemExit("Run directory must be inside project runs")
    args.run_dir.mkdir(parents=True, exist_ok=True)
    if (args.run_dir / "SOCIETY.json").exists() and not args.resume:
        raise SystemExit("Refusing to overwrite an existing simulation")
    if args.resume:
        if not (args.run_dir / "committed.json").exists() or (args.run_dir / "partial_failure.json").exists():
            raise SystemExit("Resume requires a clean committed checkpoint")
        committed = validate_commit(args.run_dir)
        original = read_json(args.run_dir / "run_config.json")
        for key in ["mode", "policy", "seed", "horizon"]:
            if original[key] != getattr(args, key):
                raise SystemExit(f"Resume {key} does not match original configuration")
        code = [row for row in source_manifest(ROOT) if row["path"].endswith(".py")]
        if original["code_manifest"] != code:
            raise SystemExit("Source code changed since this run; recovery requires the original version")
        if committed["round"] + args.steps > args.horizon:
            raise SystemExit("Resume exceeds the frozen observation horizon")
    else:
        write_json(args.run_dir / "run_config.json", {"mode": args.mode, "policy": args.policy, "seed": args.seed,
                   "horizon": args.horizon, "code_manifest": [row for row in source_manifest(ROOT) if row["path"].endswith(".py")]})
    report = {"ok": False, "mode": args.mode, "policy": args.policy, "seed": args.seed,
              "framework_version": importlib.metadata.version("agentsociety2"), "source_manifest": source_manifest(ROOT),
              "request_settings": {"max_tokens": 4096, "temperature": 0.2, "thinking": {"type": "disabled"}, "response_format": {"type": "json_object"}},
              "requested_steps": args.steps, "model": "deepseek-v4-flash", "temperature": 0.2,
              "llm_seed_supported": False, "routing": "typed_EnvLike_official_proxy_no_codegen"}
    try:
