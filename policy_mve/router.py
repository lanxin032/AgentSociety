...
class PolicyRouterActor:
    def __init__(self, run_dir, policy, seed, horizon, decision_mode, llm_client, config=None):
        self.run_dir = Path(run_dir)
        self.env_path = self.run_dir / "env" / "PolicyCaseEnv"
        self.env = get_env_module_class("PolicyCaseEnv")(policy=policy, seed=seed, horizon=horizon, config=config)
